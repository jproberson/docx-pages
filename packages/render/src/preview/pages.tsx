...
// Drawing a document beside Word's own pdf of it. Two callers ask for this: the
// suite's own documents, which `write.tsx` lays out in a fixed set, and any
// document at all, which `check.tsx` takes from the command line. Both write the
// same shape of directory, so the same server shows either.
...
export const wordFaceSet = (): FaceSet => ({
  id: "word",
...
export function authoredFaceSet(): FaceSet | null {
...
export function writeFonts(directory: string, set: FaceSet): string {
  mkdirSync(resolve(directory, FONT_DIRECTORY), { recursive: true });

  const rules = set.files.flatMap((face) => {
    if (!existsSync(face.filePath)) return [];

    const file = basename(face.filePath).replace(/\s+/g, "-");
    copyFileSync(face.filePath, resolve(directory, FONT_DIRECTORY, file));
  ...
  const path = resolve(directory, stylesheetOf(set));
...
export const FRAME_STYLES: readonly FrameStyle[] = ["hidden", "outlined"];
...
export function writePreview(
  ...
  const path = resolve(directory, pageName(each.id, set, frames));
...
const wordPageOf = (directory: string, each: ReferenceCase): string =>
  each.renderedPath === null ? "" : relative(directory, each.renderedPath);
...
export function writeBrowser(
  ...
  sets: readonly FaceSet[],
  titleOf: (each: ReferenceCase) => string,
): string {
  const options = cases
    .map(
      (each) =>
        `<option value="${escaped(each.id)}" data-word="${escaped(wordPageOf(directory, each))}">${escaped(titleOf(each))}</option>`,
    )
    .join("");
