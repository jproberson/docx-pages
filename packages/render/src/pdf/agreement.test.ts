...
describe("what a page's lines say read together", () => {
  ...
  it("agrees where every line is out by less than the tolerance", () => {
  ...
  it("counts what Word drew and we did not, and calls the page missing for it", () => {
    const drawn = asDrawn(laid, where).filter((item) => !WORDS.slice(4).includes(item.text));
    const page = agreementWith(laid, drawn, TOLERANCE_PT).pages[0];

    // The four items left agree about where they are, and the sixteen lines of ours
    // that no item spells are what the page is called missing for.
    expect(page?.shape).toBe("missing");
    expect(page?.oursAlone).toBe(16);
    expect(page?.matched).toBe(4);
  });

  it("counts the items no line of ours claimed", () => {
    const drawn = [
      ...asDrawn(laid, where),
      ...WORDS.map((word) => ({
        kind: "text" as const,
...
describe("a page with almost nothing on it", () => {
  ...
  it("is called missing where nothing on it matched at all", () => {
